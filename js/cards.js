...
export function buildCard(item, type, opts = {}) {
  ...
  if (opts.dismissible) h += `<button class="card-dismiss" data-action="dismiss-recommendation" data-id="${item.id}" data-type="${t}" data-title="${safeTitle}" data-poster="${esc(item.poster_path || '')}" data-source="${esc((item.__sources || [item.__source]).filter(Boolean).join(','))}" data-score="${Number(item.__score || 0).toFixed(3)}" aria-label="Not interested in ${safeTitle}" data-tip="Not interested"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.2"><path d="M18 6 6 18M6 6l12 12"/></svg></button>`;
