...
export async function openDetail(id, type) {
  ...
  try {
    ...
    // IMDb, the Tomatometer and Metacritic, after paint and never blocking it.
    mountScoreBadges(ct, det.external_ids?.imdb_id || '', type).catch(() => {});
...
function crewSectionHTML(cred) {
  ...
  const items = people.map(p => {
    ...
    return `<a class="cast-item" href="/person/${p.id}" data-action="open-person" data-id="${p.id}"><div class="cast-pic">${p.profile_path ? `<img src="${IMG}w185${p.profile_path}" alt="${esc(p.name)}" loading="lazy">` : `<span class="cast-initial">${esc((p.name || '?').trim()[0] || '?')}</span>`}</div><div class="cast-name">${esc(p.name)}</div><div class="cast-char" title="${esc(jobs)}">${esc(jobs)}</div></a>`;
  }).join('');
