...
function filteredListItems() {
  ...
  // The genre dropdown also holds the adult choices (js/mature-filter.js).
  const genreId = realGenre(wlGenre);
  if (genreId && genreId !== 'all') items = items.filter(w => (w.genres || []).map(String).includes(genreId));
  const adult = adultFromGenre(wlGenre);
...
function syncWLControls(baseItems) {
  ...
  if (genre) {
    ...
    genre.innerHTML = '<option value="all">All genres</option>' + [...ids]
      ...
      .map(id => `<option value="${id}">${esc(genreMap[id])}</option>`).join('') + adultGenreOptionsHTML('both');
    genre.value = wlGenre;
    if (genre.value !== wlGenre) genre.value = 'all';
...
export function renderWL() {
  ...
  if (head) {
    ...
    if (listEdit) {
      ...
      const share = `<button class="btn-glass wl-manage${shared}" data-action="share-list" data-list="${esc(active.id)}" data-tip="${active.shared ? 'Re-share list' : 'Share list'}" aria-label="${active.shared ? 'Re-share list' : 'Share list'}"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><circle cx="18" cy="5" r="3"/><circle cx="6" cy="12" r="3"/><circle cx="18" cy="19" r="3"/><line x1="8.59" y1="13.51" x2="15.42" y2="17.49"/><line x1="15.41" y1="6.51" x2="8.59" y2="10.49"/></svg></button>`;
      let manage = '';
      if (active.id !== 'watchlist') {
        const del = pendingDelete === active.id
          ...
          : `<button class="btn-glass wl-manage danger" data-action="wl-delete-list" data-tip="Delete list" aria-label="Delete list"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><polyline points="3 6 5 6 21 6"/><path d="M19 6l-1 14a2 2 0 01-2 2H8a2 2 0 01-2-2L5 6"/></svg></button>`;
        manage = `<button class="btn-glass wl-manage" data-action="wl-rename-list" data-tip="Rename list" aria-label="Rename list"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M11 4H4a2 2 0 00-2 2v14a2 2 0 002 2h14a2 2 0 002-2v-7"/><path d="M18.5 2.5a2.12 2.12 0 013 3L12 15l-4 1 1-4z"/></svg></button>${del}`;
      }
      const lockBtn = listHasPin(active.id)
        ? `<button class="btn-glass wl-manage locked" data-action="relock-list" data-list="${esc(active.id)}" data-tip="Lock now" aria-label="Lock this list now"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><rect x="4" y="11" width="16" height="10" rx="2"/><path d="M8 11V7a4 4 0 0 1 8 0v4"/></svg></button><button class="btn-glass wl-manage" data-action="open-list-pin" data-mode="change" data-list="${esc(active.id)}" data-tip="Change PIN" aria-label="Change this list's PIN"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><rect x="4" y="11" width="16" height="10" rx="2"/><path d="M8 11V7a4 4 0 0 1 8 0v4"/><path d="M12 15v2"/></svg></button><button class="btn-glass wl-manage danger" data-action="open-list-pin" data-mode="remove" data-list="${esc(active.id)}" data-tip="Remove PIN" aria-label="Remove this list's PIN"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><rect x="4" y="11" width="16" height="10" rx="2"/><path d="M8 11V7a4 4 0 0 1 6.9-2.3"/></svg></button>`
        : `<button class="btn-glass wl-manage" data-action="open-list-pin" data-mode="set" data-list="${esc(active.id)}" data-tip="Lock with a PIN" aria-label="Lock this list with a PIN"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><rect x="4" y="11" width="16" height="10" rx="2"/><path d="M8 11V7a4 4 0 0 1 8 0"/></svg></button>`;
  ...
  const refs = adultFromGenre(wlGenre) ? baseItems.map(matureRef) : [];
  ...
  if (checking.length && !adultWaiting) {
    adultWaiting = true;
    resolveMature(checking).then(() => { adultWaiting = false; if (location.pathname === '/watchlist' && adultFromGenre(wlGenre)) renderWL(); });
  }
...
export function initWatchlist() {
  ...
  onMatureToggle(on => { if (!on && realGenre(wlGenre) !== wlGenre) wlGenre = 'all'; if (location.pathname === '/watchlist') renderWL(); });
